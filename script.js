...
async function calculate() {
  ...
  const workedHolidayDates = getDateList("workedHolidayList");
  ...
  for (let day = startDay; day <= new Date(year, month + 1, 0).getDate(); day++) {
    ...
    const isWorkedHoliday = workedHolidayDates.includes(formatDate(date));

    if ((!isWeekend && !isHoliday) || isWorkedHoliday) {
      remainingWeekdays++;
    }
  }
  ...
  const totalWorkableDays = adjustedWeekdays + workedHolidayDates.length;

  const avgMins = totalWorkableDays ? Math.ceil(remainingMins / totalWorkableDays) : 0;
  ...
  if (workedHolidayDates.length > 0) {
    resultHtml += `<p>祝日出社日: ${workedHolidayDates.length}日 (${workedHolidayDates.join(', ')})</p>`;
  }

  resultHtml += `<p>実質の残り営業日: ${adjustedWeekdays}日</p>`;
  resultHtml += `<div class="result-highlight">1日平均必要時間: ${formatMinutes(avgMins)}（${totalWorkableDays}日で算出）</div>`;
